...
(quote (my . (partner . (is . (dumb)))))
; expect (my partner is dumb)

'(i can count 1 2 (3 . (4)))
; expect (i can count 1 2 (3 4))

(begin (define x (+ 1 2 3)) (cons 1 (cons 2 nil)))
; expect (1 2)

(((lambda (a b c) (lambda (x) (+ a b c x))) 1 2 3) 1)
; expect 7

((lambda (lambde lambdi) (- lambde lambdi)) 10 4)
; expect 6

(define (multi a b c d) (* a b c d))
; expect multi

(cond ((= 1 1) 'short_circuit)
           ((= 1 0) (/ 1 0))
           (else 'definitely a problem))
; expect short_circuit

(define func (mu (a b) (* a b x)))
(define func2 (lambda (x y) (func (* y y) (+ y y))))
(cond ((= (func2 1 2) 12) 'correct?)
      ((= (func2 1 2) 15) 'definitely_wrong)
      ((= (func2 1 2) 17) 'hands_so_many_hand)
      (else (func2 1 2))
)
;expect 16
...
(define f (mu (a) (* a b c)))
(define g (mu (b) (f (+ 5 b))))
(define h (lambda (c) (g (- c 10))))
(h 12)
;expect 168

(and (or 'first 'last) (and 1 4 #t '?) (and (or #f 6) 21) (or f g) g)
;expect (mu (b) (f (+ 5 b)))

(define x 1)
(define y 1)
(let ((x 2) (y x))
  (if (= x y) 'repetitive 'funkytown)
)
;expect funkytown

(if (= x y) 'grapes 'DeNero)
;expect grapes

(or (and #f #t) (or 'origami 21) (and 1 0))
;expect origami

(if (and (cons 1 2) '(cons 1 2)) (eval (define e 2.718)) 0 )
;expect 2.718

(begin (define x 12) (define y 10))
(if (= (begin '(1 2) (+ 5 6) (+ 12 0)) x) 'celebrate 'sadness)
;expect celebrate

(define x 10)
(define (fun_func yay hooray)
    (define y (/ yay hooray))
)
(define zzz (fun_func 50 5))
(if (= x y) 'yes??? 'hyperbole)
;expect yes???
; END PROBLEM 0
