...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
        parent = Frame(self.env)
        return parent.make_child_frame(self.formals,args)
        # END PROBLEM 12
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.second == nil:
            target = scheme_eval(expressions.first,env,True)
        else:
            target = scheme_eval(expressions.first,env,False)
        if not scheme_truep(target):
            return False
        if expressions.second == nil:
            return target
...
def make_let_frame(bindings, env):
    ...
    forms,args = nil,nil
    while bindings != nil:
        initial = bindings.first
        check_form(initial,2,2)
        args = Pair(scheme_eval(initial.second.first,env),args)
        forms = Pair(initial.first,forms)
